...
`run_workflow` is async and is imported into the chat route module, so it is
...
def test_chat_uses_default_session_id_when_omitted(client):
    ...
    # Act
    with patch("api.routes.chat.run_workflow", new=mock_agent):
...
def test_chat_returns_502_on_onsenbot_error(client):
    ...
    # Act
    with patch("api.routes.chat.run_workflow", new=failing):
