...
The trip-planner **agent** — the concrete query the workflow can't serve (*"plan a 3-day onsen trip"*): dynamic tool sequencing + re-planning. **Single agent first; multi-agent only if it strains** (hasn't strained yet). **Live in prod** via `trip_enabled=true`. Full design + per-PR status: [`docs/archive/v3-trip-planner-plan.md`](./docs/archive/v3-trip-planner-plan.md).
...
**Next:** see [`docs/next-steps.md`](./docs/next-steps.md) for the prioritised backlog (candidate itineraries + reasoning, the Places/billing decision that unlocks travel-time/attractions/photos, a real region-narrowing conversational gap found via user testing, and the deferred model migration). Full V3 design + per-PR status: [`docs/archive/v3-trip-planner-plan.md`](./docs/archive/v3-trip-planner-plan.md).
